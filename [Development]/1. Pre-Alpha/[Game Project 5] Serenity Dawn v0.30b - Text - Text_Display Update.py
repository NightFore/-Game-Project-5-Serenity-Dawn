...
def Setup_Loop(Button=None, Text=None, Story=None, Sprite=None, Fight=None):
    # Tools
...
def Title_Screen():
    ...
    # Buttons
    Button("Start",    Text_Button_1, 1*display_width/4, 3*display_height/4, display_width/8, display_height/12, 15, True, True, Color_Green, Color_Red, None, Prologue)
    Button("Gallery",  Text_Button_1, 2*display_width/4, 3*display_height/4, display_width/8, display_height/12, 15, True, True, Color_Green, Color_Red, None, OST_Gallery)
    Button("Debug",    Text_Button_1, 3*display_width/4, 3*display_height/4, display_width/8, display_height/12, 15, True, True, Color_Green, Color_Red, None, Main_Menu)
    ...
    while not gameExit:
        ...
        for event in Tools.events:
            if event.type == pygame.QUIT:
                Quit_Game()  
        Text_Display(Project_Title, display_width/2, display_height/4, True, Text_Title_Screen)
    ...
    for row in range(round(len(List_OST)/6)) :
        for col in range(6):
            if Music_Selection < len(List_OST):
                Button("Music %i" % (Music_Selection+1), Text_Button_1, 60+(40+display_width/8)*col, 100+(40+display_height/10)*row, display_width/8, display_height/10, 12, True, False, Color_Green, Color_Red, Music_Selection, Music_Play)
                Music_Selection += 1
...
def Game_ui_Fight():
    ...
    if Combat.Turn_Phase != "" and Combat.Turn_Phase <= 2:
        if Combat.Button_Action == False:
            ...
            Button("Attack", Text_Button_1, 640, 590, 140, 40, 6, True, True, Color_Button, Color_Red, None, Attack_Choice)
            Button("Skill",  Text_Button_1, 640, 640, 140, 40, 6, True, True, Color_Button, Color_Red, None, None)
            Button("Potion", Text_Button_1, 640, 690, 140, 40, 6, True, True, Color_Button, Color_Red, None, None)
...
# Text - Main Function
def Text_Display(msg, x, y, center, Text_Font):
...
def Text_Input():
    # Read Text
    if Story.textinput.update(Tools.events):
        # Text Input
        Story.Line_Input = Story.textinput.get_text()
        Story.textinput  = pygame_textinput.TextInput()
        ...
        if Story.Line_Input != "":
            ...
            PlayerIG.name   = Story.Line_Input
            Story.Line_Input = ""
            Story.Read       = "(NEXT)"


        # Next File
        if "(NEXT)" in Story.Read:
            Story.File.close()
            GameState.Story += 1
            Story.File = open(List_Story[GameState.Story], "r")
            Story.Read = ""

        # Event == Stop Reading
        if Story.Event == False:
            
            # Read File
            Story.Read = Story.File.readline().rstrip('\n').replace("%PlayerIG.name", ("%s" % GameState.Character[0].name))

            # Event State
            if "(EVENT)" in Story.Read:
                Story.Event = True

            # Input State
            if "(INPUT)" in Story.Read:
                Story.Read       = Story.Read.strip("(INPUT)")
                Story.Line_Input = ""
                Story.Input      = True
        ...
        # Left Side
            if "[L]" in Story.Read:
            # Character Name
                if "(NAME)" in Story.Read:
                    Story.L_Character = Story.Read.replace("(NAME)[L]", "")
                    Story.Read = Story.File.readline().rstrip('\n').replace("%PlayerIG.name", ("%s" % GameState.Character[0].name))
            # Text
                Story.L_Line[Story.L_Order]   = Story.Read.replace("[L]", "")
                Story.L_Line[Story.L_Order+1] = "(-> Press Enter)"
                Story.R_Line[Story.R_Order+1] = ""
                Story.L_Order += 1
        ...
        # Right Side
            ...
            # Character Name
                if "(NAME)" in Story.Read:
                    Story.R_Character = Story.Read.replace("(NAME)[R]", "")
                    Story.Read = Story.File.readline().rstrip('\n').replace("%PlayerIG.name", ("%s" % GameState.Character[0].name))

            # Text
                Story.R_Line[Story.R_Order]   = Story.Read.replace("[R]", "")
                Story.R_Line[Story.R_Order+1] = "(-> Press Enter)"
                Story.L_Line[Story.L_Order+1] = ""
                Story.R_Order += 1
...
def Story_Text_Display():
    ...
    # Character Name
    Text_Display(Story.L_Character, 100,  535, True, Text_Interface)
    Text_Display(Story.R_Character, 1180, 535, True, Text_Interface) 

    # Text Left
    Text_Display(Story.L_Line[1], 5, 565, False, Text_Interface)
    Text_Display(Story.L_Line[2], 5, 585, False, Text_Interface)
    Text_Display(Story.L_Line[3], 5, 605, False, Text_Interface)
    Text_Display(Story.L_Line[4], 5, 625, False, Text_Interface)
    Text_Display(Story.L_Line[5], 5, 645, False, Text_Interface)
    Text_Display(Story.L_Line[6], 5, 665, False, Text_Interface)
    Text_Display(Story.L_Line[7], 5, 685, False, Text_Interface)

    # Text Right
    Text_Display(Story.R_Line[1], 725, 565, False, Text_Interface)
    Text_Display(Story.R_Line[2], 725, 585, False, Text_Interface)
    Text_Display(Story.R_Line[3], 725, 605, False, Text_Interface)
    Text_Display(Story.R_Line[4], 725, 625, False, Text_Interface)
    Text_Display(Story.R_Line[5], 725, 645, False, Text_Interface)
    Text_Display(Story.R_Line[6], 725, 665, False, Text_Interface)
    Text_Display(Story.R_Line[7], 725, 685, False, Text_Interface)
