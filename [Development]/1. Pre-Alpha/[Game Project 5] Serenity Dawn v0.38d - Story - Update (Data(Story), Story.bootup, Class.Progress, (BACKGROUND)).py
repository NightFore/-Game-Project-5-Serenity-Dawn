...
class Setup():
    ...
    def update(self):
        ...
        if self.background != None and self.background != False:
...
class StoryIG():
    def __init__(self, state_read=True, state_input=False, path=""):
        ...
        self.story          = self.load_story()                         # Load text files
        self.bootup         = ""                                        # Run self.update() once
        self.file           = open(self.story[Progress.story], "r")     # Open the text file
    ...
    def next_file(self):
        self.file.close()
        Progress.story += 1
        self.file = open(self.story[Progress.story], "r")
...
def Main_Cutscene():
    # Setup
    Setup.update_init(music=OST_Cutscene_1_1, text=True, story=True)
