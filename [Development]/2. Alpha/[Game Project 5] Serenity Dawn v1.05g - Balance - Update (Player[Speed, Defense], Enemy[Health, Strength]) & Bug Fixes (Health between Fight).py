    ...
    def update_enemy(self, enemy=[], random_enemy=False):
        ...
        # Training Enemy
            ...
            for index in range(Enemy_Count):
                gen_enemy = list_enemy[random.randint(0, len(list_enemy)-1)]
                name = "Monster %s" % (index+1)
                avg_level, increment   = 0, 0
                for player in range(3):
                    if self.slot[player] == True:
                        increment += 1
                        avg_level += self.character[player].level
                avg_level = avg_level/increment
                self.character[index+3] = gen_enemy(name,avg_level)
                self.slot[index+3] = True
                self.death[index+3] = False
    ...
    def attack(self, target):
        ...
        Damage      : Damage value between 80% and 100% of the character strength
            ...
            (2) [Attack * (10*Attack / (10*Attack + Defense^2*Attack/100))]
                100*(1000/(1000+x^2)) = 10  (x = 100)
                10*(100/100+x^2*10))  = 1   (x = 10)
        """
        ...
        attack      = attacker.Strength * (10*attacker.Strength / (10*attacker.Strength + (defender.Defense**2)*attacker.Strength/100))
