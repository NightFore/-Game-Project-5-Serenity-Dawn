...
class Setup():
    ...
    def inventory_init(self):
        ...
        def status(index):
            print(Setup.list_text)
            if self.state_status == False:
                Text(("Class : " + Fight.character[index].Class), 450, 130, False, Text_Interface)
        ...
        if Setup.inventory == False:
            ...
            for index in range(3):
                if Fight.slot[index] == True:
                    Button(Fight.character[index].name, Text_Interface, 340, 180+150*index, 146, 38, 5, True, True, Color_Red, Color_Green, index, status)
        ...
        if Setup.result == False:
            ...
            for index in range(3):
                if self.slot[index] == True:
                    ...
                    if self.character[index].Experience >= 100:
                        self.character[index].Level += 1
                        self.character[index].Experience -= 100
