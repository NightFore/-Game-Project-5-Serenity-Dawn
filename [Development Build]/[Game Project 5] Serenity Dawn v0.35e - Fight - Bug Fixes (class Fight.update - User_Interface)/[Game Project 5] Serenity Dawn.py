...
class Fight():
    ...
    def update(self):
        ...
        # Button Commands
        if self.turn != None and self.button == True:
            print("lol")
            self.button = False
        ...
        Update : Action Point
            ...
            If the character is a player, selection buttons will appear
        """
        if self.turn == None:
            for index in range(6):
                print(self.turn)
        ...
        """
        Update : Turn Phase
            Player Phase : Show action choices to the Player
            Enemy Phase : Attack randomly a Player character
        """
        if self.turn != None:
            # Player Phase
            if self.turn < 3:
...
def Menu_Zone_1():
    ...
    # Sprite Setup
    AnimatedSprite(615,  435, True, "Data\Sprite_Button\Sprite_Button_Traning", clock.tick(FPS), 4, Debug_Fight)
    AnimatedSprite(1230, 520, True, "Data\Sprite_Button\Sprite_Button_Fight",   clock.tick(FPS), 4, None)
