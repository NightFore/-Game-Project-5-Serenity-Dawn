...
class Button():
    def __init__(self):
        ...
        self.center = []    # Center Button

        if self.center == True:
            self.x = self.x-self.w/2
            self.y = self.y-self.h/2
        self.rect   = pygame.Rect(self.x[self.index], self.y[self.index], self.w[self.index], self.h[self.index])

        self.active     = []    # Active Color
        self.inactive   = []    # Inactive Color
        self.color      = []    # Active/Inactive Button
    ...
    def Check():
        ...
        if Button.rect[Button.index].rect.collidepoint(mouse):
            self.color[Button.index] = Button.active[Button.index]
            if event.type == pygame.MOUSEBUTTONDOWN:
                if Button.action[Button.index] != None:
                    Button.action[Button.index]()
        else:
            self.color[Button.index] = Button.inactive[Button.index]

    def Update():
        # Button
        pygame.draw.rect(gameDisplay, Button.color, Button.rect[Button.index], Button.width)

        # Text
        textSurf, textRect = Text_Font(Button.text[Button.index])
        textRect.center = x, y
