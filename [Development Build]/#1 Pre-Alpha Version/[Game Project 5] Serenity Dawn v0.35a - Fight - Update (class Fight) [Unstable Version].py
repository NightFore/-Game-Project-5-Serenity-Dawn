...
class Fight():
    def __init__(self):
        ...
        # Interface Position
        self.status_bar_text_y = 2*[590,640,690]
        self.status_bar_image_y = 2*[570,620,670]
        
        self.character_x = [100,250,265,1055,905,890]
        self.character_y = [300,175,375,300,175,375]
        
        self.icon_x = 3*[10] + 3*[730]
        self.icon_y = self.status_bar_text_y
        
        self.name_x = 3*[135] + 3*[855]
        self.name_y = self.status_bar_text_y
        
        self.health_x = 3*[305] + 3*[1025]
        self.health_y = self.status_bar_text_y
    ...
    def random_enemy(self):
        for index in range(3):
            random = random.randint(0, len(list_enemy)-1)
            enemy = []
            enemy.append(list_enemy[random])
            return enemy

    def ui_fight(self):
        Text("Time: %s"  % self.time, self.time_x, self.time_y, True, Text_Inferface)
        Text("Stage: %s" % self.stage, self.stage_x, self.stage_y, True, Text_Interface)

        # Button Commands
        if self.turn in self.character:
        ...
        for index in range(6):
            if self.slot[index] == True:
                ...
                Text("%s" % self.character[i].name, self.name_x[index], self.name_y[index], True, Text_Interface)
    ...
    def attack_choice(self):
        for index in range(3, 6):
            if self.slot[index] == True and self.death[index] == False:
                ...
                Button(None, Text_Interface, sprite_rect[0]-10, sprite_rect[1]-10, sprite_rect[2]+20, sprite_rect[3]+20, 8, True, False, Color_Red, Color_Green, i, self.attack)

    def attack(self, target):
        hit = random.randint(0, 100)
        accuracy = (50+0.5*self.turn.Level) * (self.turn.Speed**2 / self.character[target].Speed**2)
        critical = (10+0.5*self.turn.Level) * self.turn.Speed * self.turn.Strength / (self.character[target].Defense * self.turn.Defense*5)

        if accuracy >= hit:
            if critical >= hit:
                self.character[target].Health += self.turn.Strength*2

            else:
                self.character[target].Health += self.turn.Strength
    ...
    def win_condition(self):
        if all(GameState.Character_Slot[3:5] and GameState.Character_Death[3:5]):
