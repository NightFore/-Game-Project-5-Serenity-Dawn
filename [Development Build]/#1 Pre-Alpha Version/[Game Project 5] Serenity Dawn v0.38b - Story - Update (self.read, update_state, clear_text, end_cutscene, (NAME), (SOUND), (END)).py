...
class StoryIG():
    def __init__(self, read=True, input_box=False, path=""):
        # Text Input
        self.textinput  = pygame_textinput.TextInput()
        self.input_line = self.textinput.get_text()

        # State
        self.path       = path                                      # Path to Text Files
        self.story      = self.load_story()                         # Open Text Files
        self.file       = open(self.story[GameState.Story], "r")    # Read File
    ...
    def end_cutscene(self):
        self.next_file(next_line=False)
        self.clear_text(left=True)
        self.clear_text(right=True)
        Tools.Current_Progress += 1
        eval(Tools.Progress[Tools.Current_Progress])()
    ...
    def next_file(self, next_line=True):
        self.file.close()
        GameState.Story += 1
        self.file = open(self.story[GameState.Story], "r")
    ...
    def load_story(self):
        """
        Loads all text files in directory. The directory must only contain text files.
        ...
        for file_name in os.listdir(self.path):
            story.append(self.path + os.sep + file_name)
        return story
StoryIG = StoryIG(path="Data\Story")
