...
class Setup():
    def __init__(self, background=False, music=False, button=False, sprite=False,  fight=False, text=False, story=False):
        # Background
...
def Title_Screen():
    # Setup
    Setup(Background_Title_Screen_1, OST_Title_Screen, button=True, text=True)
...
def Prologue_2():
    # Setup
    Setup(Background_Cutscene_1, OST_Cutscene_1_2)
...
def Prologue_3():
    # Setup
    Setup(Interface_Fight, OST_Fight_1_1)
    ...
    while not gameExit:
        for event in Tools.events:
            if event.type == pygame.QUIT:
                Quit_Game()
        Setup_Loop(Text=True, Button=True, Fight=True)

def Prologue_4():
    # Setup
    Setup(Interface_Fight, OST_Menu_Victory_1)
