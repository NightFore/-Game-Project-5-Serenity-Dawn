...
class Tools():
    def __init__(self):
        ...
        Tools.Progress          = ["Prologue_1", "Prologue_2"]
        ...
        if self.textinput.update(Tools.events):
            ...
            if self.read == True:
                ...
                if "[R]" in self.read_line:
                    ...
                    self.text_line[0][self.index[0]+1]   = ""
                    self.index[1] += 1
        self.display()
...
def Title_Screen():
    ...
    Button("Debug",    Text_Button, 3*display_width/4, 3*display_height/4, display_width/8, display_height/12, 15, True, True, Color_Green, Color_Red, None, Menu_Zone_1)
