...
class Fight():
    ...
    def update(self):
        ...
        for side in range(2):
            for index in range(3):
                index_c = index+3*side
                if self.slot[index_c] == True:
                    character = self.character[index_c]

                    if self.death[index_c] == False:
        ...
        if self.turn != None:
            # Player Phase
            if self.turn < 3:
                sprite_rect = self.character[self.turn].Sprite.get_rect(topleft=(self.character_x[0][self.turn], self.character_y[0][self.turn]))
    ...
    def update_enemy(self, enemy=[], random_enemy=False):
        ...
        if random_enemy == True:
            ...
            for player in range(3):
                if self.slot[player] == True:
                    avg_lvl.append(self.character[player].level)
            ...
            for index in range(enemy_count):
                ...
                self.character[index+3] = enemy_random(enemy_name, avg_lvl)
                self.slot[index+3]      = True
                self.death[index+3]     = False
    ...
    def attack_choice(self):
        ...
        if self.state_attack == False:
            ...
            for index in range(3, 6):
                if self.slot[index] == True and self.death[index] == False:
                    sprite_rect = self.character[index].Sprite.get_rect(topleft=(self.character_x[1][index-3], self.character_y[1][index-3]))
